/*************************************************************
 *  LED?demo  ?  PIC24FJ128GA010
 *  RD13 = ?nast?pny tryb?  |  RD6 = ?poprzedni tryb?
 *  Kolejno?? 1?9 jak w opisie (bin up ? LFSR)
 *************************************************************/
...
static uint8_t generator = 0b1110011;      /* 7?bit LFSR */
static uint8_t mode = 1;                   /* 1?9          */

static uint8_t prevS6 = 1, prevS13 = 1;    /* zbocza */
...
{
    uint8_t nowS6  = PORTDbits.RD6;   /* S6  ? wstecz */
    uint8_t nowS13 = PORTDbits.RD13;  /* S7  ? naprzód */

    __delay32(15000);                 /* ~3,75?ms */

    if (prevS13 && !nowS13) {         /* RD13 ? : nast?pny */
        mode = (mode == 9) ? 1 : mode + 1;
        resetStates();
    }
    if (prevS6 && !nowS6) {           /* RD6  ? : poprzedni */
...
/* ----------- ISR Timer?1 (identyczny poza nazwami przyc.) -- */
...
int main(void)
{
    TRISA   = 0x0000;                 /* LED?y na RA0?RA7 */
    AD1PCFG = 0xFFFF;

    TRISD |= (1<<6) | (1<<13);        /* RD6, RD13 = wej?cia */

    T1CON = 0x8030;                   /* 1:256, start */
    TMR1 = 0;  PR1 = 0x0FFF;
